...
class UaClient(object):
    ...
    def connect(self, uri):
        self.disconnect()
        print("Connecting to %s with parameters %s, %s, %s, %s" % (uri, self.security_mode, self.security_policy, self.user_certificate_path, self.user_private_key_path))
        self.client = Client(uri)
        self.client.application_uri = self.application_uri
        self.client.description = "project-machine"
    ...
    def get_node_data(self, _id_):
        node = self.client.get_node(_id_)
        try:
    ...
    def set_node_data(self, _id_, _value_):
        node = self.client.get_node(_id_)
        try:
            node.write_value(_value_)
        except Exception as ex:
            print(ex)
            raise
